...
router.post('/', async (req, res) => {
    try {
        const { Type, PurchaseDate, Supplier, Description, PurchaseCurrency, OriginalCurrencyValue, USDValue, UsefulMonths, Disposed = false, DisposalDate } = req.body;
        const connection = await (0, database_1.getConnection)();
        const result = await connection.request()
            .input('Type', Type)
...
router.put('/:id', async (req, res) => {
    try {
        const { Type, PurchaseDate, Supplier, Description, PurchaseCurrency, OriginalCurrencyValue, USDValue, UsefulMonths, Disposed, DisposalDate } = req.body;
        const connection = await (0, database_1.getConnection)();
        const result = await connection.request()
            .input('id', req.params.id)
            .input('Type', Type)
