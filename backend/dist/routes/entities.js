    ...
    try {
        ...
        try {
            const result = await pool
                ...
                .input('CurrencyCode', database_1.sql.NVarChar(10), CurrencyCode)
                .input('SSAccCode', database_1.sql.NVarChar(50), SSAccCode)
                .input('TaxAccCode', database_1.sql.NVarChar(50), TaxAccCode)
                .input('SSExpCode', database_1.sql.NVarChar(100), SSExpCode || null)
                .input('TaxExpCode', database_1.sql.NVarChar(100), TaxExpCode || null)
                .input('SalExpCode', database_1.sql.NVarChar(100), SalExpCode || null)
            ...
            if (SSExpCode !== undefined)
                created.SSExpCode = SSExpCode;
            if (TaxExpCode !== undefined)
                created.TaxExpCode = TaxExpCode;
            if (SalExpCode !== undefined)
                created.SalExpCode = SalExpCode;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        try {
            ...
            if (SSExpCode !== undefined)
                result.SSExpCode = SSExpCode;
            if (TaxExpCode !== undefined)
                result.TaxExpCode = TaxExpCode;
            if (SalExpCode !== undefined)
                result.SalExpCode = SalExpCode;
            res.json(result);
        }
    }
    catch (error) {
