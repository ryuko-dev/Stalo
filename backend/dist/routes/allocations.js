    ...
    try {
        ...
        if (PositionID) {
            ...
            const result = await pool
                .request()
                .input('ProjectID', database_1.sql.UniqueIdentifier, projectId)
                .input('ResourceID', database_1.sql.UniqueIdentifier, resourceId)
                .input('PositionID', database_1.sql.UniqueIdentifier, positionId)
                .input('PositionName', database_1.sql.NVarChar(200), positionName)
                .input('MonthYear', database_1.sql.Date, MonthYear)
                .input('AllocationMode', database_1.sql.NVarChar(10), AllocationMode || '%')
                .input('LoE', database_1.sql.Decimal(18, 2), LoE)
                .query(`
...
router.put('/:id', async (req, res) => {
    try {
        ...
        await pool
            ...
            .input('ProjectName', database_1.sql.NVarChar(200), ProjectName)
            .input('ResourceName', database_1.sql.NVarChar(200), ResourceName)
            .input('PositionName', database_1.sql.NVarChar(200), PositionName)
            .input('MonthYear', database_1.sql.Date, MonthYear)
            .input('AllocationMode', database_1.sql.NVarChar(10), AllocationMode)
            .input('LoE', database_1.sql.Decimal(18, 2), LoE)
...
router.post('/validate-positions', async (req, res) => {
    try {
        ...
        const sanitizedMonths = monthYears
            .map((m) => String(m).substring(0, 7))
            .filter((m) => monthYearRegex.test(m));
        ...
        const posParams = sanitizedMonths.map((m, i) => {
            posRequest.input(`month${i}`, database_1.sql.VarChar(7), m);
            return `@month${i}`;
        });
    ...
    `);
        ...
        const allocParams = sanitizedMonths.map((m, i) => {
            allocRequest.input(`month${i}`, database_1.sql.VarChar(7), m);
            return `@month${i}`;
        });
    ...
    `);
        ...
        const positionsToSetYes = [];
        const positionsToSetNo = [];
        ...
        const positionAllocations = new Map();
        ...
        if (allocationsToDelete.length > 0) {
            ...
            const deleteParams = allocationsToDelete.map((id, i) => {
                deleteRequest.input(`delId${i}`, database_1.sql.UniqueIdentifier, id);
                return `@delId${i}`;
            });
      ...
      `);
        ...
        if (positionsToSetYes.length > 0) {
            ...
            const yesParams = positionsToSetYes.map((id, i) => {
                yesRequest.input(`yesId${i}`, database_1.sql.UniqueIdentifier, id);
                return `@yesId${i}`;
            });
      ...
      `);
        ...
        if (positionsToSetNo.length > 0) {
            ...
            const noParams = positionsToSetNo.map((id, i) => {
                noRequest.input(`noId${i}`, database_1.sql.UniqueIdentifier, id);
                return `@noId${i}`;
            });
