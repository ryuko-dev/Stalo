...
router.post('/', async (req, res) => {
  try {
    ...
    const isValidGuid = (str: string | null | undefined): boolean => {
      if (!str) return false;
    ...
    if (existingRecord.recordset.length > 0) {
      ...
      const updateResult = await pool
        .request()
        .input('id', sql.UniqueIdentifier, existingRecord.recordset[0].ID)
        .input('Department', sql.NVarChar(200), Department || null)
        .input('EntityID', sql.UniqueIdentifier, sanitizedEntityID)
        .input('WorkingDays', sql.NVarChar(20), WorkingDays || null)
        .input('Currency', sql.NVarChar(10), Currency || null)
        .input('NetSalary', sql.Decimal(18, 2), NetSalary || null)
        .input('SocialSecurity', sql.Decimal(18, 2), SocialSecurity || null)
        .input('EmployeeTax', sql.Decimal(18, 2), EmployeeTax || null)
        .input('EmployerTax', sql.Decimal(18, 2), EmployerTax || null)
        .input('Housing', sql.Decimal(18, 2), Housing || null)
        .input('CommunicationsOther', sql.Decimal(18, 2), CommunicationsOther || null)
        .input('AnnualLeave', sql.Int, AnnualLeave || null)
        .input('SickLeave', sql.Int, SickLeave || null)
        .input('PublicHolidays', sql.Int, PublicHolidays || null)
        .input('ProjectAllocations', sql.NVarChar(sql.MAX), ProjectAllocations || null)
...
router.patch('/lock', async (req, res) => {
  try {
    ...
    const request = pool.request().input('locked', sql.Bit, locked);
