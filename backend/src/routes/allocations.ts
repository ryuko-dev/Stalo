...
// Lock/Unlock allocations for a specific month
router.patch('/lock-month/:monthYear', async (req, res) => {
  try {
    const { monthYear } = req.params;
    const { locked } = req.body; // true to lock, false to unlock

    if (monthYear === undefined || locked === undefined) {
      return res.status(400).json({ error: 'monthYear and locked status are required' });
    }

    const pool = await getConnection();

    // Debug: Check what allocations exist for this month
    const debugResult = await pool
      .request()
      .input('monthYear', `${monthYear}%`)
      .query(`
        SELECT ID, MonthYear, Locked FROM dbo.Allocation
        WHERE MonthYear LIKE @monthYear
      `);

    console.log(`Found ${debugResult.recordset.length} allocations for month ${monthYear}:`, debugResult.recordset);

    // Update allocations for this month - MonthYear is stored as YYYY-MM-DD, so we match the prefix
    const result = await pool
      .request()
      .input('monthYear', `${monthYear}%`)
      .input('locked', sql.Bit, locked ? 1 : 0)
      .query(`
        UPDATE dbo.Allocation
        SET Locked = @locked
        WHERE MonthYear LIKE @monthYear
    ...
    console.log(`Updated ${result.rowsAffected[0]} allocations to locked=${locked}`);

    res.json({ 
      success: true, 
      message: `${result.rowsAffected[0]} allocations ${locked ? 'locked' : 'unlocked'}`,
      rowsAffected: result.rowsAffected[0]
    });
  } catch (error: any) {
    console.error('Error updating allocation lock status:', error);
    res.status(500).json({ error: 'Failed to update allocation lock status', details: error.message });
  }
});

// Lock/Unlock individual allocation
router.patch('/:id/lock', async (req, res) => {
  try {
    ...
    res.json({ 
      success: true, 
      message: `Allocation ${locked ? 'locked' : 'unlocked'}`,
      locked
    });
  } catch (error: any) {
    console.error('Error updating allocation lock status:', error);
