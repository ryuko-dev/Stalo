...
router.get('/versions/:jobNo', async (req: Request, res: Response) => {
  const { jobNo } = req.params;
  
  try {
    const pool = await sql.connect(getDbConfig());
    const result = await pool.request()
      .input('jobNo', sql.NVarChar(20), jobNo)
      .query(`
        SELECT 
          Version_ID,
          Job_No,
...
router.post('/versions', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.request()
      .input('jobNo', sql.NVarChar(20), jobNo)
      .input('versionName', sql.NVarChar(100), versionName)
      .input('versionDescription', sql.NVarChar(500), versionDescription || null)
      .input('sourceType', sql.NVarChar(20), sourceType || 'Manual Edit')
      .input('sourceVersionId', sql.Int, sourceVersionId || null)
...
router.put('/versions/:versionId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isBaseline) {
      ...
      if (jobNoResult.recordset.length > 0) {
        await pool.request()
          .input('jobNo', sql.NVarChar(20), jobNoResult.recordset[0].Job_No)
    ...
    await pool.request()
      ...
      .input('versionName', sql.NVarChar(100), versionName)
      .input('versionDescription', sql.NVarChar(500), versionDescription || null)
      .input('isBaseline', sql.Bit, isBaseline || 0)
...
router.get('/data/:versionId', async (req: Request, res: Response) => {
  const { versionId } = req.params;
...
router.post('/data/:versionId', async (req: Request, res: Response) => {
  const { versionId } = req.params;
  ...
  try {
    ...
    try {
      ...
      for (const item of budgetData) {
        await transaction.request()
          .input('versionId', sql.Int, versionId)
          .input('jobNo', sql.NVarChar(20), item.jobNo)
          .input('jobTaskNo', sql.NVarChar(20), item.jobTaskNo)
          .input('budgetMonth', sql.Date, item.budgetMonth)
          .input('budgetAmount', sql.Decimal(18, 2), item.budgetAmount)
          .input('modifiedBy', sql.NVarChar(100), modifiedBy || 'System')
          .query(`
...
router.put('/data/:versionId', async (req: Request, res: Response) => {
  const { versionId } = req.params;
  ...
  try {
    ...
    try {
      for (const update of updates) {
        ...
        await transaction.request()
          .input('versionId', sql.Int, versionId)
          .input('jobNo', sql.NVarChar(20), update.jobNo)
          .input('jobTaskNo', sql.NVarChar(20), update.jobTaskNo)
          .input('budgetMonth', sql.Date, update.budgetMonth)
          .input('budgetAmount', sql.Decimal(18, 2), update.budgetAmount)
          .input('modifiedBy', sql.NVarChar(100), modifiedBy || 'System')
          .query(`
...
router.post('/data/:versionId/copy', async (req: Request, res: Response) => {
  const { versionId } = req.params;
  ...
  try {
    ...
    const result = await pool.request()
      .input('versionId', sql.Int, versionId)
      .input('sourceVersionId', sql.Int, sourceVersionId)
      .query(`
        INSERT INTO BudgetData (Version_ID, Job_No, Job_Task_No, Budget_Month, Budget_Amount, Last_Modified_By, Last_Modified_Date)
